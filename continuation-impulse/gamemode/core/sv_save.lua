...
concommand.Add("impulse_save_saveall", function(ply, cmd, args)
	...
	for v,k in pairs(ents.GetAll()) do
		if k.impulseSaveEnt then
			table.insert(savedEnts, {pos =  k.AlwaysPos or k:GetPos(), angle = k:GetAngles(), material = k:GetMaterial(), class = k:GetClass(), model = k:GetModel(), keyvalue = (k.impulseSaveKeyValue or nil)})
		end
	end
