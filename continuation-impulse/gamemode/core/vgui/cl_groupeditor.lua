...
function PANEL:ShowGroupWar()
	local group = impulse.Group.Groups[1]
	local name = LocalPlayer():GetSyncVar(SYNC_GROUP_NAME, "ERROR")
	local sheet = self:AddSheet("War", "icon16/gun.png")
	local panel = self

	local lbl = vgui.Create("DLabel", sheet)
	lbl:SetText("War Screen")
	lbl:SetFont("Impulse-Elements19-Shadow")
	lbl:SizeToContents()
	lbl:Dock(TOP)

	local groupsel = vgui.Create("DLabel", sheet)
	groupsel:SetText("Group to start war against:")
	groupsel:SetFont("Impulse-Elements19-Shadow")
	groupsel:SizeToContents()
	groupsel:Dock(TOP)

	local SelGroup = vgui.Create("DComboBox", sheet)
    SelGroup:Dock(TOP)
    SelGroup:SetText("Select Group..")

	-- print(GetGlobalString("impulse.Group.PublicGroups"))
	local GroupList = util.JSONToTable(GetGlobalString("impulse.Group.PublicGroups"))
	-- PrintTable(GroupList)
    for v,k in pairs(GroupList) do
		if v == LocalPlayer():GetSyncVar(SYNC_GROUP_NAME) then continue end
        SelGroup:AddChoice(v, k)
    end
	SelGroup.OnSelect = function( w, e, value )
		SelGroup.GroupToWar = value
	end

	local reason = vgui.Create("DTextEntry", sheet)
    reason:Dock(TOP)
    reason:SetText("")
	reason:DockMargin(0, 4, 0, 0)
	reason:SetPlaceholderText( "Reason to start war for?" )

	local start = vgui.Create("DButton", sheet)
    start:Dock(TOP)
    start:SetText("Start War")
	start:DockMargin(0, 4, 0, 0)
	start.DoClick = function()
		local GroupWarSelect = SelGroup.GroupToWar or false
		if not GroupWarSelect then
			return LocalPlayer():Notify("You need to select a group!")
		end
		Derma_Query("Are you sure you want to start a war against " .. SelGroup.GroupToWar .. "?", "impulse", "Yes", function()

			local Reason = reason:GetValue() or "No Reason."

			net.Start("impulseGroupStartWar")
			net.WriteString(GroupWarSelect) -- Group Name
			net.WriteString(Reason) -- Reason
			net.SendToServer()
	...
	if LocalPlayer().GroupEnemyData then
		for k, v in pairs(LocalPlayer().GroupEnemyData) do
			if not k then continue end

			local board = vgui.Create("DPanel", sheet)
			board:Dock(TOP)
			board:SetHeight(50)
			function board:Paint() end

			local button = vgui.Create("DButton", board)

			button.group_text = vgui.Create("DLabel", board)
			button.group_text:SetText(k)
			button.group_text:SetFont("Impulse-Elements14-Shadow")
			button.group_text:SizeToContents()
			button.group_text:Dock(LEFT)

			button:SetText("END WAR")
			button:SetFont("Impulse-Elements14-Shadow")
			button:SizeToContents()
			button:Dock(RIGHT)
			button:DockMargin(12, 12, 12, 12)
			button.DoClick = function()
				Derma_Query("Are you sure you want to end the war against " .. k .. "?", "impulse", "Yes", function()

					net.Start("impulseGroupLeaveWar")
					net.WriteString(button.group_text:GetText())
					net.SendToServer()

					panel:Remove()
				end, "No, I want death forever.")
			end
		end
	end
end
