...
timer.Simple(1, function() if not impulse.GetSetting("hud_original_hud") then
	...
	function PANEL:Teams()
		...
		for v,k in pairs(impulse.Teams.Data) do
			...
			if not selectedList.scroll then
				selectedList.scroll = selectedList:Add("DScrollPanel")
				selectedList.scroll:Dock(FILL)
				selectedList.scroll:SetHeight(90)
				selectedList:SetHeight(300)
			end
	
			local teamCard = selectedList.scroll:Add("impulseTeamCard")
			teamCard:SetTeam(v)
			teamCard.team = v
			teamCard:Dock(TOP)
			teamCard:SetHeight(90)
