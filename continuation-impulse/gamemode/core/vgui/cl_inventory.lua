...
if not impulse.GetSetting("hud_original_hud") then
	function PANEL:Init()
		...
		self:SetPos(ScrW() * .2, ScrH(), 0.00001)
		self:MoveTo(ScrW() * .2, ScrH() * .16, 1, 0, .2)
	...
	function PANEL:SetupItems()
		local itemhover = self.itemhover
		local w, h = self:GetSize()
		...
		if self.invScroll2 and IsValid(self.invScroll2) then
			self.invScroll2:Remove()
		end
	
		self.invScroll2 = vgui.Create("DScrollPanel", self.tabs)
		 self.invScroll2:SetPos(0, 0)
		 self.invScroll2:SetSize(w - math.Clamp(s, 100, 270), h - 42)
	
		self.invScroll = vgui.Create("DTileLayout", self.invScroll2)
		self.invScroll:SetBaseSize(24)
		self.invScroll:Dock(FILL)
		-- self.invScroll:SetSpaceX(1)
		-- self.invScroll:SetSpaceY(1)
		 ...
		 if localInv and table.Count(localInv) > 0 then
			 if shouldSortEq then
				 for v,k in SortedPairsByMemberValue(localInv, "sortWeight", invertSort) do
					 ...
					 local item = self:Add("impulseInventoryItem")
					-- item:Dock(TOP)
					-- item:DockMargin(0, 0, 15, 5)
					item:SetItem(k, w)
					self.invScroll:Add(item)
					...
					function item.model:OnCursorExited()
						if IsValid(itemhover) then itemhover:Remove() end
					end
			...
			self.empty = self.invScroll2:Add("DLabel", self)
		...
		self.tabs:AddSheet("Inventory", self.invScroll2)
