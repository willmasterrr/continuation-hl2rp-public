...
if not impulse.GetSetting("hud_original_hud") then
	function PANEL:Init()
		...
		local col = Color(187, 70, 181)
		local sin = sin or 0
	
		local bodyCol = Color(7, 7, 7, 238)
		function self.core:Paint(w, h)
			sin = math.Clamp(math.sin(RealTime() * 1) * 200, 10, 90)
			surface.SetDrawColor(ColorAlpha(col, sin))
			surface.SetMaterial(gradient)
			surface.DrawTexturedRect(0, 0, 200, h)
	
			surface.SetDrawColor(bodyCol) -- menu body
			surface.DrawTexturedRect(0,0,400,h) -- left body
		...
		copyrightLabel:SetText("Powered by impulse\nCopyright 2i.games "..year.."\nimpulse version: "..impulse.Version.."\nPowered by Continuation Schema (By willmasterr, https://github.com/willmasterrr/continuation-hl2rp-public)")
		...
		timer.Simple(0, function()
			...
			if not steamworks.IsSubscribed("3446733132") then
				Derma_Query("You are not subscribed to the server content!\nIf you do not subscribe you will experience missing textures and errors.\nAfter subscribing, rejoin the server.",
					"impulse",
					"Subscribe",
					function()
						gui.OpenURL("https://steamcommunity.com/sharedfiles/filedetails/?id=3436298912")
					end,
					"No thanks")
			end
