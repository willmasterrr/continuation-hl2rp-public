...
timer.Simple(1, function() if not impulse.GetSetting("hud_original_hud") then
	...
	function PANEL:SetTeam(teamID)
		...
		self.colour = Color(TeamCol.r, TeamCol.g, TeamCol.b, 200)
		...
		if self.bodygroups then
			 timer.Simple(0, function()
				...
				if teamData.subMaterial then
					self.modelIcon.subMats = {}
					for v,k in pairs(team_div["subMaterial"]) do
						ent:SetSubMaterial(v - 1, k)
						self.modelIcon.subMats[v] = k
					end
				end
			end)
		 end
	...
	function PANEL:Paint(w,h)
		...
		surface.SetDrawColor(self.colour)
		 ...
		 surface.DrawTexturedRect(1,1,w-1,h)
		...
		surface.SetTextPos(80,10)
		surface.DrawText(self.name)
	
		-- team requirements
		surface.SetTextPos(80,25)
