...
if not impulse.GetSetting("hud_original_hud") then
		...
		function self.count:Think()
			if panel.Count > 1 and panel.Count != self.lastCount then
				...
				if panel.IsRestricted or panel.Item.Illegal then
					self:SetPos(47 + wShift, 66)
				else
					self:SetPos(64 + wShift, 66)
				end
			end
		end
	...
	function PANEL:OnMousePressed(keycode)
		...
		if self.Basic then
			...
			if keycode == MOUSE_RIGHT and count > 1 then
				...
				opt = m:AddOption("Move All", moveItems)
				opt.Moving = self.Count
	...
	function PANEL:Paint(w, h)
		...
		if item then
			...
			draw.SimpleText(self.Weight.."kg", "Impulse-Elements16", w - 2, 2, color_white, TEXT_ALIGN_RIGHT)
...
function PANEL2:SetItem(netitem, wide)
	local direct = self.ContainerType
	local item = impulse.Inventory.Items[(direct and netitem) or netitem.id]
	self.Item = item
...
function PANEL2:OnMousePressed(keycode)
	if self.Disabled then
		return
	end
...
vgui.Register("impulseStorageInventoryItem", PANEL2, "DPanel")