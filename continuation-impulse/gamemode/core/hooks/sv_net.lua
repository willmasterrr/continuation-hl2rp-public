...
util.AddNetworkString("impulseDoorBuyApts")
util.AddNetworkString("impulseDoorSellApts")
util.AddNetworkString("impulseDoorAddApts")
util.AddNetworkString("impulseDoorRemoveApts")

util.AddNetworkString("impulseGroupStartWar")
util.AddNetworkString("impulseGroupLeaveWar")

util.AddNetworkString("impulseGroupBroadcastWarData")
...
net.Receive("impulseATMWithdraw", function(len, ply)
	...
	if ply:CanAffordBank(amount) then
		...
		ReqwestLog({
			{
				description = ply:Nick() .. "(" .. ply:SteamName() .. ")(" .. ply:SteamID() .. ") has withdraw " .. amount .. " from his ATM.",
...
net.Receive("impulseATMDeposit", function(len, ply)
	...
	if ply:CanAfford(amount) then
		...
		ReqwestLog({
			{
				description = ply:Nick() .. "(" .. ply:SteamName() .. ")(" .. ply:SteamID() .. ") has deposited " .. amount .. " in his ATM.",
				color = 16711680, -- Red color (in decimal)
			}
		})
	else
...
net.Receive("impulseDoorBuy", function(len, ply)
	...
	if traceEnt:GetSyncVar(SYNC_DOOR_APARTMENT, nil) then return end
...
net.Receive("impulseMixTry", function(len, ply)
	...
	timer.Simple(time, function()
		...
		if IsValid(ply) and ply:Alive() and IsValid(benchEnt) and ply:CanMakeMix(mixClass) then
			...
			if amount > 1 then
				ply:Notify("You have crafted "..mixClass.OutputAmount.." "..item.Name.."s.")		
			else
...
net.Receive("impulseGroupDoDelete", function(len, ply)
	...
	for v,k in pairs(groupData.Members) do
		...
		if IsValid(targEnt) then
			...
			targEnt:SetSyncVar(SYNC_GROUP_COL, nil, true)
...
net.Receive("impulseDoorBuyApts", function(len, ply)
	local door = net.ReadEntity()
	if (ply.nextDoorBuy or 0) > CurTime() then return end
	local tbl = door:GetSyncVar(SYNC_DOOR_APARTMENT, nil)
	tbl = util.JSONToTable(door:GetSyncVar(SYNC_DOOR_APARTMENT, nil))
	if (not ply:Alive()) or (ply:GetSyncVar(SYNC_ARRESTED, false)) then return end
	if (not tbl) or (tbl.isapt != true) or (ply.CurApartmentDoor) then return end

	if IsValid(door) and ply:CanBuyDoor(door:GetSyncVar(SYNC_DOOR_OWNERS, nil), door:GetSyncVar(SYNC_DOOR_BUYABLE, true)) and hook.Run("CanEditDoor", ply, door) != false then
		if ply:CanAfford(impulse.Config.DoorPrice) then
			ply:TakeMoney(impulse.Config.DoorPrice)
			ply:SetDoorMaster(door)
			ply:SetDoorTwins(door)

			local value = {
				["apt_ent"] = door:EntIndex(),
				["ownsapt"] = true,
				["apt_data"] = door:GetSyncVar(SYNC_DOOR_APARTMENT),
			}
			ply:SetSyncVar(SYNC_DOOR_APARTMENT_OWNED, util.TableToJSON(value), true)

			ply.CurApartmentDoor = door

			ply:Notify("You are now the owner of Apartment " .. door:GetSyncVar(SYNC_DOOR_APARTMENT_NUM, "000"))

			hook.Run("PlayerPurchaseDoor", ply, door)
		else
			ply:Notify("You cannot afford to buy this Apartment.")
		end
	end
	ply.nextDoorBuy = CurTime() + 1
end)

net.Receive("impulseDoorSellApts", function(len, ply)
	local door = ply.CurApartmentDoor

	if (ply.nextDoorSell or 0) > CurTime() or (not door) then return end
	if not ply:Alive() or ply:GetSyncVar(SYNC_ARRESTED, false) then return end
	local tbl = door:GetSyncVar(SYNC_DOOR_APARTMENT, nil)
	tbl = util.JSONToTable(door:GetSyncVar(SYNC_DOOR_APARTMENT, nil))
	if (not ply:Alive()) or (ply:GetSyncVar(SYNC_ARRESTED, false)) then return end
	if (not tbl) or (tbl.isapt != true) then return end

	if IsValid(door) and ply:IsDoorOwner(door:GetSyncVar(SYNC_DOOR_OWNERS, nil)) and door:GetDoorMaster() == ply and hook.Run("CanEditDoor", ply, door) != false then
		
		local partners = door:GetSyncVar(SYNC_DOOR_OWNERS)
		if (partners) then
			for k, v in pairs(partners) do
				if v == ply:EntIndex() then continue end
				Entity(v):SetSyncVar(SYNC_DOOR_APARTMENT_OWNED, nil, true)
			end
		end

		ply:RemoveDoorMaster(door)
		ply:RemoveDoorTwins(door)
		ply:GiveMoney(impulse.Config.DoorPrice - 2)

		ply:SetSyncVar(SYNC_DOOR_APARTMENT_OWNED, nil, true)

		ply.CurApartmentDoor = nil

		ply:Notify("You have sold apartment " .. door:GetSyncVar(SYNC_DOOR_APARTMENT_NUM, "000"))

		hook.Run("PlayerSellDoor", ply, door)
	end
	ply.nextDoorSell = CurTime() + 1
end)

net.Receive("impulseDoorAddApts", function(len, ply)
	local target = net.ReadEntity()
	local traceEnt = ply.CurApartmentDoor

	if (ply.nextDoorChange or 0) > CurTime() then return end
	ply.nextDoorChange = CurTime() + 0.5

	if not ply:Alive() or ply:GetSyncVar(SYNC_ARRESTED, false) then return end
	local tbl = traceEnt:GetSyncVar(SYNC_DOOR_APARTMENT, nil)

	if (not tbl) then return end

	tbl = util.JSONToTable(traceEnt:GetSyncVar(SYNC_DOOR_APARTMENT, nil))

	if (tbl.isapt != true) then return end
	...
	if IsValid(traceEnt) and ply:IsDoorOwner(owners) and traceEnt:GetDoorMaster() == ply then
		...
		local value = { 
			["apt_ent"] = traceEnt:EntIndex(),
			["ownsapt"] = true,
			["apt_data"] = traceEnt:GetSyncVar(SYNC_DOOR_APARTMENT),
		}
		target:SetSyncVar(SYNC_DOOR_APARTMENT_OWNED, util.TableToJSON(value), true)

		target:SetDoorTwins(traceEnt, true)



		ply:Notify("You have added "..target:Nick().." to your apartment.")
		target:Notify(ply:Nick() .. " Has added you to their apartment. (" .. traceEnt:GetSyncVar(SYNC_DOOR_APARTMENT_NUM) .. ")")

		hook.Run("PlayerAddUserToDoor", ply, owners)
	end
end)

net.Receive("impulseDoorRemoveApts", function(len, ply)
	local target = net.ReadEntity()
	local traceEnt = ply.CurApartmentDoor
	...
	if IsValid(traceEnt) and ply:IsDoorOwner(traceEnt:GetSyncVar(SYNC_DOOR_OWNERS, nil)) and traceEnt:GetDoorMaster() == ply then
		...
		target:RemoveDoorUser(traceEnt)
		target:RemoveDoorTwins(traceEnt, true)

		target:SetSyncVar(SYNC_DOOR_APARTMENT_OWNED, nil, true)

		ply:Notify("You have removed " .. target:Nick() .. " from your apartment.")
		target:Notify(ply:Nick() .. " Has removed you from their apartment. (" .. traceEnt:GetSyncVar(SYNC_DOOR_APARTMENT_NUM) .. ")")
	end
end)

net.Receive("impulseGroupLeaveWar", function(len, ply)
	local enemy_name = net.ReadString()
	local name = ply:GetSyncVar(SYNC_GROUP_NAME, nil)

	if (not ply:GroupHasPermission(99)) or (not enemy_name) or (not name) or (not impulse.Group.Groups[enemy_name]) then
		return
	end

	if ply.impulseData and ply.impulseData.WarData then
		local serverdata_ply = util.TableToJSON(ply.impulseData.WarData)

		if not serverdata_ply[enemy_name] then
			return
		end
	end
	...
	ReqwestLog({
		{
			description = ply:Nick() .. "(" .. ply:SteamName() .. ")(" .. ply:SteamID() .. ")(" .. name .. ") has ended a war against " .. enemy_name .. ".",
			color = 16711680, -- Red color (in decimal)
		}
	})

--	impulse.Group.Groups

	local data = impulse.Group.Groups[name]
	local enemy_data = impulse.Group.Groups[enemy_name]

	for k, v in pairs(data["Members"]) do
		local ent = player.GetBySteamID(k)
		if not ent:IsValid() then return end

		local data_togive = util.JSONToTable(ent.impulseData.WarData)
		data_togive[enemy_name] = nil

		ent.impulseData.WarData = util.TableToJSON(data_togive)
        ent:SaveData()

		ent:Notify("The war against " .. enemy_name .. " has ended.")
		net.Start("impulseGroupBroadcastWarData")
		net.WriteString(ent.impulseData.WarData)
		net.Send(ent)
	end

	for k, v in pairs(enemy_data["Members"]) do
		local ent = player.GetBySteamID(k)
		if not ent:IsValid() then return end

		local enemy_data_togive = util.JSONToTable(ent.impulseData.WarData)
		enemy_data_togive[name] = nil

		ent.impulseData.WarData = util.TableToJSON(enemy_data_togive)
        ent:SaveData()

		ent:Notify("The war against " .. name .. " has ended.")
		net.Start("impulseGroupBroadcastWarData")
		net.WriteString(ent.impulseData.WarData)
		net.Send(ent)
	end
...
net.Receive("impulseGroupStartWar", function(len, ply)
	local enemy_name = net.ReadString()
	local reason4war = net.ReadString() or "No Reason."
	local name = ply:GetSyncVar(SYNC_GROUP_NAME, nil)
...
--	impulse.Group.Groups
	if ply.impulseData and ply.impulseData.WarData then
		...
		if serverdata_ply[enemy_name] then
			return ply:Notify("You're already in a war with " .. enemy_name .. "!")
		end
	end

	ReqwestLog({
		{
			description = ply:Nick() .. "(" .. ply:SteamName() .. ")(" .. ply:SteamID() .. ")(" .. name .. ") has started a war against " .. enemy_name .. "with the reason " .. reason4war,
			color = 16711680, -- Red color (in decimal)
		}
	})
	...
	local data_togive = {}
	data_togive[enemy_name] = true

	local enemy_data_togive = {}
	enemy_data_togive[name] = true
	...
	for k, v in pairs(data["Members"]) do
		...
		ent:Notify("A war has started against the group " .. enemy_name .. " for the reason " .. reason4war .. "!")
		net.Start("impulseGroupBroadcastWarData")
		net.WriteString(ent.impulseData.WarData)
		net.Send(ent)
	end
	...
	for k, v in pairs(enemy_data["Members"]) do
		...
		ent:Notify("A war has started against the group " .. name .. " for the reason " .. reason4war .. "!")
		net.Start("impulseGroupBroadcastWarData")
		net.WriteString(ent.impulseData.WarData)
		net.Send(ent)
	end
