...
function GM:ScoreboardShow()
	...
	if not impulse.GetSetting("hud_original_hud") then
		...
		impulse_scoreboard:SetAlpha(0)
		impulse_scoreboard:AlphaTo(255, .5, 0)
	else
		impulse_scoreboard = vgui.Create("impulseScoreboard")
	end
end
...
function GM:ScoreboardHide()
	...
	if not impulse.GetSetting("hud_original_hud") then
		...
		if impulse_scoreboard:GetAlpha() != 255 then
			speed = 0.05
		end

		impulse_scoreboard:AlphaTo(0, speed, 0, function()
...
function GM:DefineSettings()
	...
	impulse.DefineSetting("hud_originallogo", {name="Original Impulse Logo (Rejoin to take effect!)", category="HUD", type="tickbox", default=false})
	impulse.DefineSetting("hud_original_hud", {name="Original Impulse HUD (Rejoin to take effect!)", category="HUD", type="tickbox", default=false})
	...
	impulse.DefineSetting("firstperson_fov", {name="Viewmodel FOV", category="View", type="slider", default=0, minValue=0, maxValue=24})
...
-- local smoothAngles = smoothAngles or Angle(0, 0, 0)
...
local float = 2
local function CalcViewModelLag(weapon, vm, origin, angles, original_angles, original_pos)

	do
		local raiseTarg = 0

		if not LocalPlayer():IsWeaponRaised() then
			raiseTarg = 100
		end

		local frac = (LocalPlayer().raiseFraction or 0) / 100
		local rot = weapon.LowerAngles or loweredAngles
		angles:RotateAroundAxis(angles:Up(), rot.p * frac)
		angles:RotateAroundAxis(angles:Forward(), rot.y * frac)
		angles:RotateAroundAxis(angles:Right(), rot.r * frac)
		LocalPlayer().raiseFraction = Lerp(FrameTime() * 2, LocalPlayer().raiseFraction or 0, raiseTarg)
	end

	-- do
	-- 	local func = weapon.GetViewModelPosition
	-- 	if (func) then
	-- 		local pos, ang = func( weapon, original_pos*1, original_angles*1 )
	-- 		origin = pos or origin
	-- 		angles = ang or angles
	-- 	end

	-- 	func = weapon.CalcViewModelView
	-- 	if (func) then
	-- 		local pos, ang = func( weapon, viewModel, origin*1, angles*1, original_pos*1, original_angles*1 )
	-- 		origin = pos or origin
	-- 		angles = ang or angles
	-- 	end
	-- end
	...
	if (FrameTime() != 0.0) then
		...
		if ( (flDiff > float) and (float > 0.0) ) then
			local flScale = flDiff / float;
		...
		VectorMA(origin, 15.0, vDifference * 1.0, origin);
	end
...
local function doLag(weapon, vm, oldPos, oldAng, pos, ang)
	if (!IgnoreList[weapon:GetClass()]) and (LocalPlayer():KeyDown(IN_ATTACK2) and (not weapon.NoIronsights)) or (weapon.IgnoreSway) then
		return
	end
	...
	if (IsValid(weapon) and weapon.GetIronSights and weapon:GetIronSights()) then
		...
		CalcViewModelLag(weapon, vm, pos, ang, oldAng, oldPos)
	end
end

hook.Add("CalcViewModelView", "ImpulseHookCalcViewModelView", doLag)
...
-- 		end
-- 	end
-- 	-- smoothAngles = LerpAngle(FrameTime() * 1, LocalPlayer():AlignAngles(oldEyeAng, vm_angles), vm_angles)
...
function GM:OnContextMenuOpen()
	...
	if not input.IsKeyDown(KEY_LALT) then
		if IsValid(impulse_inventory) then return end
...
function GM:OnContextMenuClose()
	...
	if not impulse.GetSetting("hud_original_hud") then
		if IsValid(impulse_inventory) then
			impulse_inventory:MoveTo(ScrW() * .2, ScrH(), .5, 0, 1, function()
