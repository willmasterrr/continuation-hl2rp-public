...
function GM:DoPlayerDeath(ply, attacker, dmginfo)
	...
	ragdoll.wearing_hide_mask = attacker:GetSyncVar(SYNC_COS_FACE) == 4
	...
	hook.Run("OnRadgollCreation", ragdoll, ply, attacker)
...
local function LoadButtons()
	...
	-- print(impulse.Config.Buttons) -- hope this works
...
function ADVDupeIsAllowed(ply, class, entclass) -- adv dupe 2 can be easily exploited, this fixes it. you must have the impulse version of AD2 for this to work
...
function GM:LongswordMeleeHit(ply)
	if not ply:GetActiveWeapon().IgnoreStrengthLevelUP then
