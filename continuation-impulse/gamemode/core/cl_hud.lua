...
local function GetTeamClassButBetter(ply)
	if not impulse.Teams.Data[ply:Team()] then return "" end

	local classRef = impulse.Teams.Data[ply:Team()].ClassRef
	local plyClass = ply:GetSyncVar(SYNC_CLASS, nil)
...
local DrawOverheadInfo = function(target, alpha)
	...
	local groupcol = target:GetSyncVar(SYNC_GROUP_COL, '{"r":255,"b":255,"a":255,"g":255}') or '{"r":255,"b":255,"a":255,"g":255}'

	local GroupEnemyData = LocalPlayer().GroupEnemyData

	local col = ColorAlpha(team.GetColor(target:Team()), alpha)

	if (not LocalPlayer():IsCP() and not target:IsCP()) then
		if GroupEnemyData and GroupEnemyData[group] and group and rank then
			draw.DrawText("("..group.." - "..rank..")", "Impulse-Elements16-Shadow", pos.x, pos.y - 15, ColorAlpha(util.JSONToTable(groupcol), alpha), 1)
		elseif myGroup and group and rank and group == myGroup then
			draw.DrawText("("..group.." - "..rank..")", "Impulse-Elements16-Shadow", pos.x, pos.y - 15, ColorAlpha(util.JSONToTable(groupcol), alpha), 1)
		end
	...
	elseif (target:Team() == TEAM_CWU and target:GetTeamClass() != nil) then
		draw.DrawText(GetTeamClassButBetter(target), "Impulse-Elements18-Shadow", pos.x, pos.y + 15, col, 1)
	end
end
...
local function DrawDoorInfo(target, alpha)
	...
	if LocalPlayer():CanBuyDoor(doorOwners, doorBuyable) and !target:GetSyncVar(SYNC_DOOR_APARTMENT, nil) then
		draw.DrawText("Ownable door (F2)", "Impulse-Elements18-Shadow", pos.x, pos.y, col, 1)
	end
end

local DrawEntInfo =
function(target, alpha)
	local pos = target.LocalToWorld(target, target:OBBCenter()):ToScreen()
	local scrW = ScrW()
	local scrH = ScrH()
...
function GM:HUDPaint()
	...
	if not impulse.GetSetting("hud_original_hud") then
		surface.SetDrawColor(Color(team.GetColor(lpTeam).r, team.GetColor(lpTeam).g, team.GetColor(lpTeam).b, 15))
