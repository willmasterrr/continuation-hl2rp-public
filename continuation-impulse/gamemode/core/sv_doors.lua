...
function meta:SetDoorTwins(door, roommate)
	if roommate then
		if (door.ConnectedDoors) then
			for k, v in pairs(door.ConnectedDoors) do
				self:SetDoorUser(k)
			end
		end
	else
		if (door.ConnectedDoors) then
			for k, v in pairs(door.ConnectedDoors) do
				self:SetDoorMaster(k)
			end
		end
	end
end

function meta:RemoveDoorTwins(door, roommate)
	if roommate then
		if (door.ConnectedDoors) then
			for k, v in pairs(door.ConnectedDoors) do
				self:RemoveDoorUser(k)
			end
		end
	else
		if (door.ConnectedDoors) then
			for k, v in pairs(door.ConnectedDoors) do
				self:RemoveDoorMaster(k)
			end
		end
	end
end
...
function meta:SetDoorUser(door)
	...
	if table.Count(doorOwners) >= 9 then
		local owner = door.MasterUser
        owner:AchievementGive("ach_ourhouse")
	end
