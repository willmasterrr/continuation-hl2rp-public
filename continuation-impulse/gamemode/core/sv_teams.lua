...
function meta:SetTeam(teamID, forced)
	...
	-- if teamData.cp and self.impulseData.CombineBan then
	-- 	return
	-- end
	...
	if teamData.subMaterial then
		for v,k in pairs(teamData.subMaterial) do
	...
	self:SetSyncVar(SYNC_CLASS, nil, true) -- SetLocalSyncVar
	self:SetSyncVar(SYNC_RANK, nil, true) -- SetLocalSyncVar
...
function meta:SetTeamClass(classID, skipLoadout)
	...
	self:SetSyncVar(SYNC_CLASS, classID, true) -- SetLocalSyncVar

	if (self.WearingRebelOutfit) then
		self.WearingRebelOutfit = false
	end
...
function meta:SetTeamRank(rankID)
	...
	self:SetSyncVar(SYNC_RANK, rankID, true) -- SetLocalSyncVar
...
function meta:HasTeamWhitelist(team, level)
	...
	local whitelist = self.Whitelists[tostring(team)]
