...
hook.Add("HUDPaint", "impulseOpsHUD", function()
	...
	if LocalPlayer():IsAdmin() and LocalPlayer():GetMoveType() == MOVETYPE_NOCLIP then
		...
		if LocalPlayer():IsTrialGamemaster() and impulse.GetSetting("admin_esp") then
			...
			for v,k in pairs(player.GetAll()) do
				...
				local hp =  "HP: " .. k:Health() .. "/" .. k:GetMaxHealth()
				local armor = "ARMR: " .. k:Armor()
				local col1 = Color(29, 29, 29)
				if k:IsAdmin() and k:GetMoveType() == MOVETYPE_NOCLIP and k:GetNoDraw() then
					draw.SimpleText("** In Observer Mode **", "Impulse-Elements18-Shadow", pos.x, pos.y - 14, Color(255, 0, 0), TEXT_ALIGN_CENTER)
				end
				...
				draw.SimpleText(k:SteamID(), "Impulse-Elements16-Shadow", pos.x, pos.y + 34, impulse.Config.InteractColour, TEXT_ALIGN_CENTER)

				draw.RoundedBox(0, pos.x - 50, pos.y + 55, 105, 17, col1)
				draw.RoundedBox(0, pos.x - 50, pos.y + 76, 105, 17, col1)

				draw.RoundedBox(0, pos.x - 50, pos.y + 55, k:Health() * 1.05, 17, Color(0, 170, 0))
				draw.RoundedBox(0, pos.x - 50, pos.y + 76, k:Armor() * 1.05, 17, Color(0, 0, 170))
				draw.SimpleText(hp, "Impulse-Elements16-Shadow", pos.x, pos.y + 55, color_white, TEXT_ALIGN_CENTER)
				draw.SimpleText(armor, "Impulse-Elements16-Shadow", pos.x, pos.y + 76, color_white, TEXT_ALIGN_CENTER)
			end
		end
