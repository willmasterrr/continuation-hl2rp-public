    ...
    onRun = function(ply, arg, rawText)
        ...
        if targ and IsValid(targ) then
            ...
            if targ == ply then
                for v,k in pairs(player.GetAll()) do
                    if k:IsLeadAdmin() then
                        k:AddChatText(Color(135, 206, 235), "[ops] Moderator "..ply:SteamName().." set their hunger to "..hp..".")
                    end
                end
            end
        else
...
local setXPCommand = {
    ...
    onRun = function(ply, arg, rawText)
        ...
        if targ and IsValid(targ) then
            targ:SetXP(tonumber(hp))
            ply:Notify("You have set "..targ:Nick().."'s xp to "..hp..".")

            if targ == ply then
                for v,k in pairs(player.GetAll()) do
                    if k:IsLeadAdmin() then
                        k:AddChatText(Color(135, 206, 235), "[ops] Moderator "..ply:SteamName().." set their xp to "..hp..".")
                    end
                end
            end
        else
...
local setmoneyCommand = {
    ...
    onRun = function(ply, arg, rawText)
        ...
        if targ and IsValid(targ) then
            targ:SetMoney(tonumber(hp))
            ply:Notify("You have set "..targ:Nick().."'s money to "..hp..".")

            if targ == ply then
                for v,k in pairs(player.GetAll()) do
                    if k:IsLeadAdmin() then
                        k:AddChatText(Color(135, 206, 235), "[ops] Moderator "..ply:SteamName().." set their money to "..hp.." on " .. targ:Name() .. ".")
                    end
                end
            end
        else
...
local fixlegsCommand = {
    description = "fixes the legs of a player.",
    requiresArg = false,
    adminOnly = true,
    onRun = function(ply, arg, rawText)
        local targ = impulse.FindPlayer(arg[1])
        ...
        if targ and IsValid(targ) then
            ...
            if targ == ply then
                for v,k in pairs(player.GetAll()) do
                    if k:IsLeadAdmin() then
                        k:AddChatText(Color(135, 206, 235), "[ops] Moderator "..ply:SteamName().." fixed legs to "..targ:Nick()..".")
                    end
                end
            end
        else
