local broadcastCommand = {
	...
	onRun = function(ply, arg, rawText)
		if (ply:GetPos() - impulse.Config.BroadcastPos):LengthSqr() <= ((impulse.Config.BroadcastDistance or impulse.Config.WhisperDistance) ^ 2) and ply:Alive() then

			if string.len(rawText) > 465 then return ply:Notify("This broadcast is too long! (Max Length: 465)") end
...
local vortessenceCommand = {
	description = "Send a message over to every vortigaunt with the vortessence effect.",
	requiresArg = true,
	requiesAlive = true,
	onRun = function(ply, arg, rawText)
		if ply:Alive() and ply:Team() == TEAM_VORT and ply.UsedVortessence then

			for v,k in pairs(player.GetAll()) do	
				if (k:Team() != TEAM_VORT) or (not k.UsedVortessence) then continue end
				k:SendChatClassMessage(54, rawText, k)
			end
		else
			ply:Notify("You are not near the broadcasting station.")
		end
...
impulse.RegisterChatCommand("/hivelink", vortessenceCommand)
...
if CLIENT then
    ...
    impulse.RegisterChatClass(56, function(message)
        impulse.customChatFont = "Impulse-ChatRadio"
...
local function StationaryRadioCommand(ply, message)
	...
	if ent and IsValid(ent) and ent:GetClass() == "impulse_radio" then
