...
local function DrawCombineObjective(citycode)
    local x = ScrW() * .5
    local y = ScrH() * .2
    local order = CP_OBJECTIVE.message or ""

    if citycode then
        if citycode == CODE_JW then
            order = "JUDGEMENT WAIVER is now in effect. Sentencing is now discretionary."
        elseif citycode == CODE_AJ then
            order = "AUTONOMOUS JUDGEMENT now in effect. Capital prosecution is discretionary."
        end
    end
    
    surface.SetFont("DFTitle")
    local minX = surface.GetTextSize(citycode and (impulse.Dispatch.CityCodes[citycode][1].." IN EFFECT") or "PRIORITY OBJECTIVE")
    surface.SetFont("DFSubTitle")
    local orderSize = surface.GetTextSize(order)
    local wide = math.Clamp(orderSize, minX, ScrW()) + 30

    surface.SetDrawColor(ColorAlpha(messagesCol, 180))
    surface.DrawRect(x - (wide / 2), y, wide, 100)
    ...
    CP_OBJECTIVE.textPos = CP_OBJECTIVE.textPos or 0
    CP_OBJECTIVE.nextText = CP_OBJECTIVE.nextText or CurTime() + textSpeed

    if CurTime() > CP_OBJECTIVE.nextText then
        CP_OBJECTIVE.textPos = CP_OBJECTIVE.textPos + 1
        CP_OBJECTIVE.nextText = CurTime() + textSpeed
    end

    if citycode then
        draw.SimpleText(string.sub(impulse.Dispatch.CityCodes[citycode][1].." IN EFFECT", 1, CP_OBJECTIVE.textPos), "DFTitle", x, y, impulse.Dispatch.CityCodes[citycode][2], TEXT_ALIGN_CENTER)
        draw.SimpleText(string.sub("ATTENTION GROUND TEAMS", 1, CP_OBJECTIVE.textPos), "DebugFixed", x, y + 30, nil, TEXT_ALIGN_CENTER)

        draw.SimpleText(string.sub(order:upper(), 1, CP_OBJECTIVE.textPos), "DFSubTitle", x, y + 60, nil, TEXT_ALIGN_CENTER)
        return
    end

    draw.SimpleText(string.sub("PRIORITY OBJECTIVE", 1, CP_OBJECTIVE.textPos), "DFTitle", x, y, CMB_OVERLAY_COLS[1], TEXT_ALIGN_CENTER)
    if CP_OBJECTIVE.sender then
        draw.SimpleText(string.sub("ISSUED BY UNIT "..CP_OBJECTIVE.sender, 1, CP_OBJECTIVE.textPos), "DebugFixed", x, y + 30, nil, TEXT_ALIGN_CENTER)
    else
        draw.SimpleText(string.sub("ISSUED BY DISPATCH", 1, CP_OBJECTIVE.textPos), "DebugFixed", x, y + 30, nil, TEXT_ALIGN_CENTER)
    end
    draw.SimpleText(string.sub(order:upper(), 1, CP_OBJECTIVE.textPos), "DFSubTitle", x, y + 60, nil, TEXT_ALIGN_CENTER)
end
...
local function DrawCombineHUD()
    ...
    if CP_OBJECTIVE then
        if CP_OBJECTIVE.endTime < CurTime() then
            CP_OBJECTIVE = nil
        else
            DrawCombineObjective(CP_OBJECTIVE.citycode)
        end
    end
end
