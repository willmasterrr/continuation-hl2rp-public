	...
	for v,k in pairs(team.GetPlayers(lp:Team())) do
		...
		if not squads[sqd] then
			...
			squads[sqd].Header.OnMousePressed = function(self, code)
				...
				if code == MOUSE_RIGHT and ((isLeader and sqd != squad) or (lp:Team() == TEAM_CP and lp:GetTeamRank() >= RANK_SQL))  then
					local m = DermaMenu()
					...
					if lp:GetTeamRank() >= RANK_SQL then
